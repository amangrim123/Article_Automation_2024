...
def Get_Content(mysql,dir):
    ...
    all_Content.extend(webs[0:5])
    post_count.append(len(all_Content))
    if len(all_Content) > 0:
        print("the all_content Len = ",len(all_Content))
        return all_Content
    else:
        return False
...
def Get_token(dir,mydb):
    ...
    mydb.commit()
    mycursor.close()
    # print(f"Update token for {dest__id}")
    return token    
...
def main():
    while True:
        ...
        print("Total Site =",len(myresult))
        for dir in myresult:
            try:    
                print("Site Url = ",dir[1])
                ...
                if Site_Token:
                    ...
                    print("Site Is Not Working = ",dir[1])        
                time.sleep(10)
            except Exception as ee:
                print(ee)
                error_log.exception(f"{datetime.now()} - Error: {str(ee)}\n") 
        mydb.close()
        print("Program Sleep For 10 Minutes")
        time.sleep(600)
