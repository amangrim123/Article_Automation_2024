...
class QuotesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in self.al_web:
           if "feed" in url[1]:
                remove_tags = [url[9],url[10],url[11]]
                yield scrapy.Request(url=url[1],headers=headers, meta={'bfw_id':url[0],"Destination_id":url[6],"title12":url[2],"feature_imgg":url[4],"catego":url[5],'contte':url[3],'Remove_Tags':remove_tags}, callback=self.parse)
           else:
               yield scrapy.Request(url=url[1], headers=headers, meta={'bfw_id':url[0],"Destination_id":url[6],"title12":url[2],"feature_imgg":url[4],"catego":url[5],'contte':url[3],'date_path':url[8],'article_link_path':url[7]}, callback=self.parsehtml)
    ...
    def parse(self, response):
        ...
        for quote in response.xpath('//item')[0:6]:
            ...
            if self.SavedDate<date_time_obj:
                ...
                print("this is url from feed = ",url)
                bfwid=(response.meta['bfw_id'])
                dest_id=(response.meta['Destination_id'])
                yield scrapy.Request(url=url,headers=headers,meta={'bfw_id':bfwid,"uurl":url,"Destination_id":dest_id,"title13":title00,"feature_imgg13":response.meta['feature_imgg'],"catego13":category00,"cantnt":response.meta['contte'],"remove_tags":Remove_tags}, callback=self.find_feature_image)
    ...
    def find_feature_image(self, response):
        ...
        try:
            ...
            sel = Selector(text=content_of_article)    

            for i_xpath in response.meta["remove_tags"]:
                if i_xpath != None:
                    print("this is single xpath = ",i_xpath)
                    # Extract and remove the elements matching the XPath
                    elements_to_remove = sel.xpath(i_xpath).extract()
                    for element in elements_to_remove:
                        content_of_article = content_of_article.replace(str(element), '') 
            ...
            self.mydb.commit()
                ...
                error_log.exception(f"{datetime.now(),urll} - Error: {str(e)}\n")
    ...
    def parse_article(self, response):
        ...
        print("This is Article Url = ",response.url)
        ...
        if date_object:
            ...
            sel = Selector(text=content_is)

            # Define the XPath you want to remove
            # Replace with your desired XPath

            for i_xpath in response.meta["remove_tags"]:
                print(f"this is {i_xpath} and type is {type(i_xpath)}")
                if i_xpath != None:
                    print("this is single xpath = ",i_xpath)
                    # Extract and remove the elements matching the XPath
                    elements_to_remove = sel.xpath(i_xpath).extract()
                    for element in elements_to_remove:
                        content_is = content_is.replace(str(element), '')          
